...
static unsigned char get_mixsel_value(unsigned int mixer, unsigned int channel, int mixRef)
{
    mixRef = mixRef + 1;
    unsigned char data[64];
    usb_audio_class_get(CUR, mixRef, channel, usb_mixers->usb_mixSel[0].id, 1, (unsigned char *)data);
    //usb_audio_class_get(CUR, mixRef, channel, usb_mixers->usb_mixSel[mixer].id, 1, (unsigned char *)data);
    return data[0];
}
...
static int get_mixer_info(const unsigned char *data, int length, unsigned int mixer_index)
{
    ...
    while (interface_len)
    {
        ...
        if (interface_type == USB_CS_INTERFACE && unit_type == USB_MIXER_UNIT_TYPE)
        {
            ...
            usb_mixers->usb_mixSel[i].id = usb_mixers->usb_mixer[mixer_index].input_connections[0];
  ...
  if ( TSTATUS_SUCCESS != st ) {
    ...
    if(h)
    {
        ...
        for(int i = 0; i < usb_mixers->num_usb_mixers; i++)
        {
            ...
            for (int j = 0; j < usb_mixers->usb_mixer[0].num_outputs;j++)
            {
                for(int k = 0;k < usb_mixers->usb_mixSel[i].numOutputs; k++)
                {
                /* Get value from device */
                int inputChan = get_mixsel_value(i, k, j);

                usb_mixers->usb_mixSel[j].state[k] = inputChan;
...
unsigned char usb_mixsel_get_state(unsigned int mixer, unsigned int channel, int mixRef)
{
    //return get_mixsel_value(mixer,channel, mixRef);
    return usb_mixers->usb_mixSel[mixRef].state[channel];
}

void usb_mixsel_set_state(unsigned int mixer, unsigned int dst, unsigned int src, int mixRef)
{
    // write to device

    usb_audio_class_set(CUR, mixRef, dst, usb_mixers->usb_mixSel[mixRef].id, 1, (unsigned char *)&src);

    // update object state
    usb_mixers->usb_mixSel[mixRef].state[dst] = src;
    ...
    for(int i = 0;i < 18;i++)
        printf("%d ",usb_mixers->usb_mixSel[1].state[i]);
    printf("\n");
}
