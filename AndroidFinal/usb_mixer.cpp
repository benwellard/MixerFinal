...
static int find_xmos_device(unsigned int id, int filed)
{
    ...
    while ((dev = devs[i++]) != NULL)
    {
            ...
            (desc.idProduct == XMOS_L2_AUDIO2_PID)))
        {
            if (found == id)
            {
                if (libusb_open2(dev, &devh, filed) < 0)
                {
                    return -10;
                }
                else
        ...
        for(int i = 0; i < usb_mixers->num_usb_mixers; i++)
        {
            ...
            for (int j = 0; j < 8; j++)
            {
                for(int k = 0;k < usb_mixers->usb_mixSel[i].numOutputs;k++)
                {
                    ...
                    int inputChan = get_mixsel_value(i,k,j+1);

                    usb_mixers->usb_mixSel[j+1].state[k] = inputChan;
...
void usb_mixsel_set_state(unsigned int mixer, unsigned int dst, unsigned int src, int mixRef)
{
    mixRef--;
    // write to device
    usb_audio_class_set(CUR, mixRef, dst, usb_mixers->usb_mixSel[mixer].id, 1, (unsigned char *)&src);
